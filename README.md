...
| `outbox` | Two-way: drop a JSON file `{text}` or `{file, caption}` into `<dir>/outbox/` and the next poll posts it to the group, then moves the file to `outbox/sent/`. Long text is split on paragraph breaks and numbered `(1/3)`. Three failures move a file to `outbox/failed/` and report it — a reply is never dropped silently or sent twice. |
...
| POST | `/pipelines/run` | Poll all pipelines now (also flushes each outbox) |
